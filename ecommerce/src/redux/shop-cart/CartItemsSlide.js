...
export const cartItemsSlice = createSlice({
    ...
    reducers: {
        addItem: (state, action) => {
            ...
            const duplicate = state.value.filter(e => e.productId === newItem.productId && e.color === newItem.color && e.size === newItem.size)
        ...
        removeItem: (state, action) => {
            ...
            state.value = state.value.filter(e => e.productId !== item.productId || e.color !== item.color || e.size !== item.size)
            localStorage.setItem('cartItems', JSON.stringify(state.value.sort((a, b) => a.id > b.id ? 1 : (a.id < b.id ? -1 : 0))))
        },
    },
})
